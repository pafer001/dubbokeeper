...
appDependencies.directive("appDependencies", function () {
    return {
        ...
        controller: function ($scope, $dialog, $theme) {
            ...
            $scope.styles = [{
                color: $window.find(".provider").css("backgroundColor"),
                faveShape: 'ellipse'
            }, {
                color: $window.find(".consumer").css("backgroundColor"),
                faveShape: 'ellipse'
            }, {color: $window.find(".p-and-c").css("backgroundColor"), faveShape: 'ellipse'}];
            ...
            var initRending = function () {
                ...
                if ($scope.graph && $scope.graph.nodes) {
                    ...
                    for (var i = 0; i < $scope.graph.nodes.length; i++) {
                        var node = {
                            data: {
                                id: $scope.graph.nodes[i].name,
                                name: $scope.graph.nodes[i].name,
                                weight: 65,
                                faveColor: $scope.styles[$scope.graph.nodes[i].category].color,
                                faveShape: $scope.styles[$scope.graph.nodes[i].category].faveShape
                            }
                        }
                        nodes.push(node);
                    }
                    ...
                    for (var i = 0; i < $scope.graph.links.length; i++) {
                        var edge = {
                            data: {
                                source: $scope.graph.links[i].source,
                                target: $scope.graph.links[i].target,
                                faveColor: $window.find(".label-info").css("backgroundColor"),
                                strength: 90
                            }
                        };
                        edges.push(edge);
